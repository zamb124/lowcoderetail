...
class BaseDataAccessManager(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    def __init__(
        self, model_name: str, http_client: Optional[httpx.AsyncClient] = None
    ):
    ...
    def broker(self) -> "BrokerTaskProxy":
        if self._broker_instance is None:
            ...
            self._broker_instance = BrokerTaskProxy(
                dam_instance=self, model_name=self.model_name
            )
    ...
    def _get_filter_class(self) -> Type[BaseFilter]:
        try:
            ...
            if filter_cls:
                if issubclass(filter_cls, BaseFilter):
                    if not hasattr(filter_cls, "Constants") or not hasattr(
                        filter_cls.Constants, "model"
                    ):
                        constants_class = type(
                            "Constants",
                            (object,),
                            {"model": self.model, "search_model_fields": []},
                        )
                        wrapper_name = f"{filter_cls.__name__}WithConstants"
                        filter_cls = type(
                            wrapper_name, (filter_cls,), {"Constants": constants_class}
                        )
                        try:
                            filter_cls.model_rebuild(force=True)  # type: ignore
                        except Exception:
                            pass
                    return filter_cls
                else:
                    logger.warning(
                        f"Registered filter_cls {filter_cls.__name__} for {self.model_name} is not a subclass of BaseFilter. Falling back."
                    )
            else:
                logger.debug(
                    f"No specific filter registered for {self.model_name}. Using DefaultFilter."
                )
        except ConfigurationError:
            logger.warning(
                f"Model '{self.model_name}' not found in registry for filter. Using DefaultFilter."
            )
        except Exception as e:
            logger.exception(
                f"Error getting filter class for {self.model_name}. Using DefaultFilter. Error: {e}"
            )
        ...
        try:
            default_runtime_filter.model_rebuild(force=True)
        except Exception:
            pass
    ...
    async def list(
        ...
        *,
        ...
        logger.debug(
            f"Base DAM LIST: {self.model_name}, Direction: {direction}, Input Cursor: {cursor}, Limit: {limit}"
        )
        if not hasattr(self, "model") or not self.model:
            raise ConfigurationError(
                f"Model class not set for manager {self.__class__.__name__}"
            )
        if not hasattr(self.model, "lsn"):
            raise ValueError(
                f"Cursor pagination requires 'lsn' attribute on model {self.model.__name__}"
            )
        ...
        if isinstance(filters, Mapping) or isinstance(filters, BaseFilter):
            ...
            if isinstance(filters, Mapping):
                logger.debug(
                    f"Received dict filters. Validating against {actual_filter_cls.__name__}."
                )
                try:
                    ...
                    raise HTTPException(
                        status_code=422,
                        detail=f"Invalid filter parameters: {ve.errors()}",
                    )
                except Exception as e:
                    raise HTTPException(
                        status_code=500,
                        detail=f"Internal error processing filters: {e}",
                    )
            elif isinstance(filters, BaseFilter):
                if not isinstance(filters, actual_filter_cls):
                    logger.warning(
                        f"Received filter object of type {type(filters).__name__}, but expected {actual_filter_cls.__name__}. Proceeding."
                    )
                filter_obj = filters
        elif filters is not None:
            raise TypeError(
                f"Unsupported filter type: {type(filters)}. Expected Filter object or dict."
            )

        statement = base_statement
        if filter_obj:
            statement = filter_obj.filter(statement)
            is_default_filter = isinstance(
                filter_obj, DefaultFilter
            ) or filter_obj.__class__.__name__.endswith("DefaultRuntimeFilter")
            if (
                not is_default_filter
            ):  # Применяем сортировку только из кастомных фильтров
                ...
                if constants and hasattr(constants, "model"):
                    try:
                        ...
                        sort_result_select = filter_obj.sort(
                            sqlmodel_select(constants.model)
                        )  # type: ignore
                        if hasattr(sort_result_select, "_order_by_clauses"):
                            ...
                            logger.warning(
                                f"Could not extract _order_by_clauses from sort result of {type(filter_obj).__name__}"
                            )
                    except Exception as e:
                        logger.warning(
                            f"Could not apply sort from custom filter {type(filter_obj).__name__}: {e}"
                        )
                else:
                    logger.warning(
                        f"Custom filter {type(filter_obj).__name__} is missing 'Constants.model'. Cannot apply sorting."
                    )
                logger.debug(
                    f"Applied custom sort from {type(filter_obj).__name__}: {order_by_clauses}"
                )
        ...
        logger.debug(
            f"Executing query: {statement.compile(compile_kwargs={'literal_binds': True}) if hasattr(statement, 'compile') else 'SQLModel select object'}"
        )
        try:
            ...
            raise HTTPException(
                status_code=500, detail="Database error during list operation."
            )
        ...
        logger.debug(
            f"List result for {self.model_name}: Count={count}, NextCursor={output_next_cursor}"
        )
    ...
    async def create(self, data: Union[CreateSchemaType, Dict[str, Any]]) -> ModelType:
        ...
        if isinstance(data, dict):
            if create_schema_cls is None:
                raise ConfigurationError(
                    f"CreateSchema not defined for {self.model.__name__}, cannot validate dict."
                )
            try:
            ...
            logger.warning(
                f"CreateSchema not defined for {self.model.__name__}, using provided BaseModel."
            )
            ...
            raise TypeError(
                f"Unsupported data type for creating {self.model.__name__}: {type(data)}. Expected {expected_type} or dict."
            )
    ...
    async def update(
        ...
        db_item_from_db = await self.get(
            item_id
        )  # Используем self.get() чтобы получить объект в текущей сессии
        if not db_item_from_db:
            raise HTTPException(
                status_code=404, detail=f"{self.model_name} with id {item_id} not found"
            )
        ...
        if isinstance(data, dict):
            if update_schema_cls:
                try:
                    ...
                    update_payload = validated_update_schema.model_dump(
                        exclude_unset=True
                    )
                except ValidationError as ve:
                    raise HTTPException(status_code=422, detail=ve.errors())
            else:
                update_payload = data  # Если схемы нет, используем dict как есть
            ...
            logger.warning(
                f"UpdateSchema not defined for {self.model.__name__}, using provided BaseModel for update."
            )
            ...
            raise TypeError(
                f"Unsupported data type for updating {self.model.__name__}: {type(data)}. Expected {expected_type} or dict."
            )
        ...
        db_item_prepared, updated = await self._prepare_for_update(
            db_item_from_db, update_payload
        )

        if not updated:
            logger.debug(
                "No actual changes detected during update, returning original item."
            )
        ...
        try:
            ...
            logger.warning(
                f"DAM Update IntegrityError for {self.model_name} {item_id}: {e}"
            )
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Failed to update {self.model.__name__} due to internal error: {e}",
            )
    ...
    async def delete(self, item_id: UUID) -> bool:
        ...
        if db_item:
            ...
            try:
                ...
                logger.warning(
                    f"DAM Delete IntegrityError for {self.model_name} {item_id}: {e}"
                )
                self._handle_integrity_error(e, context="delete")
                return False  # Не дойдет
            except Exception as e:
                logger.exception(
                    f"DAM Delete Error for {self.model_name} {item_id}: {e}"
                )
                raise HTTPException(
                    status_code=500,
                    detail=f"Failed to delete {self.model.__name__} due to internal error.",
                )
        else:
            raise HTTPException(
                status_code=404, detail=f"{self.model_name} with id {item_id} not found"
    ...
    async def _prepare_for_create(self, validated_data: CreateSchemaType) -> ModelType:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail=f"Error preparing model instance: {e}"
            )

    async def _prepare_for_update(
        self, db_item: ModelType, update_payload: Dict[str, Any]
        ...
        for key, value in update_payload.items():
            if hasattr(db_item, key):
                ...
                if current_value != value:
                    logger.debug(
                        f"  Updating field '{key}' from {repr(current_value)} to {repr(value)}"
                    )
                    try:
                        ...
                        raise HTTPException(
                            status_code=400, detail=f"Error setting field '{key}': {e}"
                        )
            else:
                logger.warning(
                    f"  Attribute '{key}' not found on model {self.model.__name__} during update preparation."
                )
        ...
        logger.debug(
            f"_prepare_for_update finished for ID {item_id_str}. Updated: {updated}"
        )
        return db_item, updated

    async def _prepare_for_delete(self, db_item: ModelType) -> None:
        logger.debug(
            f"_prepare_for_delete for {self.model_name} ID {getattr(db_item, 'id', 'N/A')}"
        )
        pass
    ...
    def _handle_integrity_error(
        ...
        logger.warning(
            f"Handling IntegrityError (orig: {type(orig_exc).__name__}) during {context}"
        )

        if isinstance(
            orig_exc, UniqueViolationError
        ) or UniqueViolationError.__name__ in str(orig_exc):
            status_code = 409
            ...
            if (
                error_detail_pg
                and "Key (" in error_detail_pg
                and ")=" in error_detail_pg
            ):
                try:
                    field_name = error_detail_pg.split("(")[1].split(")")[0]
                except Exception:
                    pass
            detail = f"Conflict: Value for '{field_name}' already exists."
            if constraint_name != "unknown":
                detail += f" (Constraint: {constraint_name})"
        elif isinstance(
            orig_exc, NotNullViolationError
        ) or NotNullViolationError.__name__ in str(orig_exc):
            status_code = 400
            column_name = getattr(orig_exc, "column_name", "unknown")  # type: ignore
            detail = f"Bad Request: Field '{column_name}' cannot be null."
        elif isinstance(
            orig_exc, ForeignKeyViolationError
        ) or ForeignKeyViolationError.__name__ in str(orig_exc):
            status_code = 400
