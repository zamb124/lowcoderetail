...
def test_default_filter_field_definitions():
    ...
    assert get_origin(fields["id__in"].annotation) is Union or get_origin(fields["id__in"].annotation) is Optional # Optional это Union[T, None]
    id_in_args = get_args(fields["id__in"].annotation)
    assert any(get_origin(arg) is list and get_args(arg)[0] is uuid.UUID for arg in id_in_args if arg is not type(None))
    ...
    assert get_origin(fields["company_id__in"].annotation) is Union or get_origin(fields["company_id__in"].annotation) is Optional
    company_id_in_args = get_args(fields["company_id__in"].annotation)
    assert any(get_origin(arg) is list and get_args(arg)[0] is uuid.UUID for arg in company_id_in_args if arg is not type(None))
...
def test_default_filter_applies_created_at_gte(sync_session: SQLModelSession):
    ...
    item1 = FilterTestModel(id=uuid4(), name="Old", company_id=uuid4(), created_at=item1_time, updated_at=item1_time, lsn=20)
    item2 = FilterTestModel(id=uuid4(), name="Mid", company_id=uuid4(), created_at=item2_time, updated_at=item2_time, lsn=21)
    item3 = FilterTestModel(id=uuid4(), name="New", company_id=uuid4(), created_at=item3_time, updated_at=item3_time, lsn=22)
...
def test_default_filter_applies_search(sync_session: SQLModelSession):
    ...
    sync_session.add_all([
        FilterTestModel(id=uuid4(), name="Alpha Search",  company_id=uuid4(), lsn=30),
