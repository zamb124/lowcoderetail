...
async def test_get_optional_token(auth_header_value: Optional[str], expected_token: Optional[str]):
...
def mock_app() -> FastAPI:
    ...
    if not hasattr(app, 'state'):
        app.state = mock.Mock() # Простой мок, если FastAPI не создает state по умолчанию
...
async def test_app_http_client_lifespan_handles_no_initial_state_attr(mock_app: FastAPI):
    ...
    assert getattr(mock_app.state, 'http_client', 'AttributeMissing') is None # Должен быть None или отсутствовать