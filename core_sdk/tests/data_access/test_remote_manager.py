...
from respx import MockRouter # MockRouter - это то, что мы используем для respx_mock.api или respx_mock.route()

from core_sdk.registry import RemoteConfig
from core_sdk.data_access.remote_manager import RemoteDataAccessManager
from core_sdk.exceptions import ServiceCommunicationError, ConfigurationError # ConfigurationError используется
...
@pytest.fixture
def remote_item_manager(remote_config: RemoteConfig, mock_http_client: httpx.AsyncClient) -> RemoteDataAccessManager[Item, ItemCreate, ItemUpdate]:
...
# --- Тесты ---
async def test_remote_get_success(remote_item_manager: RemoteDataAccessManager, respx_mock: MockRouter): # ИЗМЕНЕНО
    item_id = uuid.uuid4()
    ...
    respx_mock.get(f"{MOCKED_API_BASE_URL}/{item_id}").respond(200, json=expected_item_data)
...
async def test_remote_get_not_found(remote_item_manager: RemoteDataAccessManager, respx_mock: MockRouter): # ИЗМЕНЕНО
    item_id = uuid.uuid4()
    respx_mock.get(f"{MOCKED_API_BASE_URL}/{item_id}").respond(404)
    item = await remote_item_manager.get(item_id)
    assert item is None

async def test_remote_get_server_error(remote_item_manager: RemoteDataAccessManager, respx_mock: MockRouter): # ИЗМЕНЕНО
    item_id = uuid.uuid4()
    respx_mock.get(f"{MOCKED_API_BASE_URL}/{item_id}").respond(500, json={"detail": "Internal Server Error"})
    with pytest.raises(HTTPException) as exc_info:
        await remote_item_manager.get(item_id)
    assert exc_info.value.status_code == 500

async def test_remote_list_success(remote_item_manager: RemoteDataAccessManager, respx_mock: MockRouter): # ИЗМЕНЕНО
...
async def test_remote_list_with_params(remote_item_manager: RemoteDataAccessManager, respx_mock: MockRouter): # ИЗМЕНЕНО
    route = respx_mock.get(url__regex=rf"{MOCKED_API_BASE_URL}\?.*").respond(200, json={"items": [], "next_cursor": None, "limit":10, "count":0})
    await remote_item_manager.list(limit=10, cursor=100, filters={"name__like": "test", "value": 5})
    assert route.called
...
async def test_remote_create_success(remote_item_manager: RemoteDataAccessManager, respx_mock: MockRouter): # ИЗМЕНЕНО
    create_data = ItemCreate(name="New Remote Item", value=50)
    item_id = uuid.uuid4()
    mock_response_data = {"id": str(item_id), "name": "New Remote Item", "value": 50, "lsn": 1}
    # URL для создания - MOCKED_API_BASE_URL + "/" (RemoteServiceClient добавляет / если нужно)
    route = respx_mock.post(f"{MOCKED_API_BASE_URL}/").respond(201, json=mock_response_data)
...
async def test_remote_create_validation_error(remote_item_manager: RemoteDataAccessManager, respx_mock: MockRouter): # ИЗМЕНЕНО
    create_data = ItemCreate(name="Bad Data")
    mock_error_response = {"detail": [{"loc": ["body", "value"], "msg": "field required"}]}
...
async def test_remote_update_success(remote_item_manager: RemoteDataAccessManager, respx_mock: MockRouter): # ИЗМЕНЕНО
    item_id = uuid.uuid4()
    update_data = ItemUpdate(description="Updated Description")
    mock_response_data = {"id": str(item_id), "name": "Existing", "description": "Updated Description", "lsn": 2}
    route = respx_mock.put(f"{MOCKED_API_BASE_URL}/{item_id}").respond(200, json=mock_response_data)
...
async def test_remote_update_not_found(remote_item_manager: RemoteDataAccessManager, respx_mock: MockRouter): # ИЗМЕНЕНО
    item_id = uuid.uuid4()
...
async def test_remote_delete_success(remote_item_manager: RemoteDataAccessManager, respx_mock: MockRouter): # ИЗМЕНЕНО
    item_id = uuid.uuid4()
...
async def test_remote_delete_already_deleted_is_success(remote_item_manager: RemoteDataAccessManager, respx_mock: MockRouter): # ИЗМЕНЕНО
    item_id = uuid.uuid4()
    respx_mock.delete(f"{MOCKED_API_BASE_URL}/{item_id}").respond(404)
    success = await remote_item_manager.delete(item_id)
    assert success is True

async def test_remote_delete_server_error(remote_item_manager: RemoteDataAccessManager, respx_mock: MockRouter): # ИЗМЕНЕНО
    item_id = uuid.uuid4()
...
async def test_remote_manager_with_auth_token(remote_config: RemoteConfig, mock_http_client: httpx.AsyncClient, respx_mock: MockRouter): # ИЗМЕНЕНО
    auth_token = "test_bearer_token"
    manager = RemoteDataAccessManager(
        remote_config=remote_config, http_client=mock_http_client, model_cls=Item,
        read_schema_cls=ItemRead, auth_token=auth_token
    )
    item_id = uuid.uuid4()
    route = respx_mock.get(f"{MOCKED_API_BASE_URL}/{item_id}").respond(200, json={"id": str(item_id), "name": "Auth Item", "lsn":1})
