...
from core_sdk.data_access.broker_proxy import _serialize_arg, _deserialize_broker_result, BrokerTaskProxy
...
from core_sdk.registry import ModelRegistry # Не используется напрямую в этих юнит-тестах, но может быть нужен для интеграционных
...
def test_serialize_arg_dict():
    ...
    assert serialized_dict == {"key1": str(uid), "key2": {"name": "InDict", "age": 20}, "key3": "simple"}
...
def test_deserialize_broker_result_pydantic_sqlmodel():
    ...
    data = {"id": str(item_id_uuid), "name": "Deserialized ItemRead", "lsn": 100, "description": "Test SQLModel"}
...
class MockDamForProxy(BaseDataAccessManager[Item, ItemCreate, ItemUpdate]):
    ...
    read_schema = ItemRead
    def __init__(self, model_name="ProxyTestItem"): super().__init__(model_name=model_name)
    async def get(self, item_id: uuid.UUID, some_kwarg: str = "default") -> Optional[ItemRead]: # pragma: no cover
    ...
    async def create(self, data: ItemCreate) -> ItemRead: # pragma: no cover
        # Этот метод не будет вызван напрямую в юнит-тестах BrokerTaskProxy
        return ItemRead(id=uuid.uuid4(), name=data.name, description=data.description, value=data.value, lsn=2)
...
def broker_proxy(mock_dam_instance_for_proxy: MockDamForProxy) -> BrokerTaskProxy:
    return BrokerTaskProxy(dam_instance=mock_dam_instance_for_proxy, model_name="ProxyTestItem")
...
async def test_broker_proxy_method_call_success(
    ...
    worker_raw_return_value = {"id": str(item_id_to_get), "name": expected_name, "lsn": 1, "description": None, "value": None}
    mock_result_handle = mock_taskiq_result_factory(return_value=worker_raw_return_value)
    mock_kiq = mock.AsyncMock(return_value=mock_result_handle)
    monkeypatch.setattr(execute_dam_operation, "kiq", mock_kiq)

    result = await broker_proxy.get(item_id_to_get, some_kwarg="via_proxy", _broker_timeout=5) # type: ignore
...
async def test_broker_proxy_timeout(
    ...
    with pytest.raises(TimeoutError) as exc_info: # BrokerTaskProxy преобразует TaskiqResultTimeoutError в TimeoutError
        await broker_proxy.get(item_id_to_get, _broker_timeout=0) # type: ignore
    assert "did not complete within 0 seconds" in str(exc_info.value) # Проверяем сообщение из BrokerTaskProxy
    assert mock_result_handle._wait_result_called
...
async def test_broker_proxy_wait_result_raises_taskiq_error(
    ...
    with pytest.raises(CoreSDKError) as exc_info:
        await broker_proxy.get(item_id_to_get) # type: ignore
    assert "Taskiq error during async execution" in str(exc_info.value) # Сообщение из BrokerTaskProxy
