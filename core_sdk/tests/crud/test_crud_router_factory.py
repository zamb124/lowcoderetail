...
from core_sdk.data_access import BaseDataAccessManager, DataAccessManagerFactory, get_dam_factory
...
from fastapi_filter.contrib.sqlalchemy import Filter as BaseSQLAlchemyFilter # Для issubclass
from sqlmodel import SQLModel, Field as SQLModelField

# Используем тестовые модели и схемы из общего conftest.py для SDK
from core_sdk.tests.conftest import (
    Item, ItemCreate, ItemUpdate, ItemRead, ItemFilter,
    CrudFactoryItem, CrudFactoryItemCreate, CrudFactoryItemUpdate,
    CrudFactoryItemRead, CrudFactoryItemFilter, # Используем эти для тестов CRUD factory
    CrudSimpleItem, CrudSimpleItemRead
)
...
def test_factory_init_success(): # manage_registry_for_crud_tests уже отработала
    factory = CRUDRouterFactory(model_name="CrudFactoryItem", prefix="/cfitems", tags=["CFTest"])
    ...
    assert factory.filter_cls is CrudFactoryItemFilter # CrudFactoryItemFilter зарегистрирован в conftest
    assert isinstance(factory.router, APIRouter)

def test_factory_init_model_not_found_raises_error():
    with pytest.raises(ConfigurationError, match="Configuration Error: Model name 'NonExistent' not found in registry. Available models"): # Ожидаем оригинальный регистр
...
def test_factory_init_default_filter_created_if_none_registered():
    ...
    assert hasattr(factory.filter_cls.Constants, 'ordering_field_name') # Проверяем наличие
...
def test_factory_generates_all_routes_if_deps_provided():
    ...
    factory = CRUDRouterFactory(
        model_name="CrudFactoryItem", prefix="/items",
        list_deps=deps, get_deps=deps, create_deps=deps, update_deps=deps, delete_deps=deps
    )
    paths = {route.path for route in factory.router.routes}
    assert "/items" in paths

    methods_for_root = {m for r in factory.router.routes if r.path == "/items" for m in r.methods}
    methods_for_id = {m for r in factory.router.routes if r.path == "/items/{item_id}" for m in r.methods}
...
def test_factory_skips_routes_if_deps_are_none():
    factory = CRUDRouterFactory(
        model_name="CrudFactoryItem", prefix="/items",
        list_deps=[Depends(dep_func_ok)], get_deps=None, create_deps=[Depends(dep_func_ok)],
        update_deps=None, delete_deps=[Depends(dep_func_ok)]
    )
    has_get_id_route = any(route.path == "/{item_id}" and "GET" in route.methods for route in factory.router.routes)
    has_put_id_route = any(route.path == "/{item_id}" and "PUT" in route.methods for route in factory.router.routes)
    assert not has_get_id_route
    assert not has_put_id_route
    assert any(route.path == "/items" and "GET" in route.methods for route in factory.router.routes)

def test_factory_skips_create_if_no_create_schema():
    # CrudSimpleItem зарегистрирован без create_schema_cls в conftest
    factory = CRUDRouterFactory(model_name="CrudSimpleItem", prefix="/simple", create_deps=[Depends(dep_func_ok)])
    has_post_route = any(route.path == "" and "POST" in route.methods for route in factory.router.routes)
...
def test_factory_skips_update_if_no_update_schema():
    ...
    factory = CRUDRouterFactory(model_name="CrudSimpleItem", prefix="/simple", update_deps=[Depends(dep_func_ok)])
    has_put_route = any(route.path == "/{item_id}" and "PUT" in route.methods for route in factory.router.routes)
    assert not has_put_route

# --- Интеграционные тесты для сгенерированных эндпоинтов ---
@pytest.fixture
def app_with_crud_router(mock_dam_factory: mock.Mock) -> FastAPI: # manage_registry_for_crud_tests не нужна, т.к. manage_model_registry_sdk - autouse
    factory = CRUDRouterFactory(
        model_name="CrudFactoryItem", prefix="/crud_items", # Используем модель со всеми схемами
        list_deps=[], get_deps=[], create_deps=[], update_deps=[], delete_deps=[]
    )
    app = FastAPI()
...
async def test_generated_create_endpoint(client: TestClient, mock_dam: mock.AsyncMock):
    ...
    mock_dam.create.return_value = CrudFactoryItemRead(id=item_id_create, name=post_data["name"], description=post_data["description"])
...
async def test_generated_get_endpoint(client: TestClient, mock_dam: mock.AsyncMock):
    ...
    mock_dam.get.return_value = CrudFactoryItemRead(id=item_id_get, name="Fetched Item", description=None)
...
async def test_generated_get_endpoint_not_found(client: TestClient, mock_dam: mock.AsyncMock):
...
async def test_generated_update_endpoint(client: TestClient, mock_dam: mock.AsyncMock):
    ...
    mock_dam.update.return_value = CrudFactoryItemRead(id=item_id_update, name="Updated CRUD Item", description=None)
...
async def test_generated_delete_endpoint_fails_returns_500(client: TestClient, mock_dam: mock.AsyncMock):
