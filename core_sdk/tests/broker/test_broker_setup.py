...
def test_broker_is_inmemory_when_env_is_not_set(monkeypatch: pytest.MonkeyPatch, caplog):
...
def test_broker_is_redis_when_env_is_prod_and_redis_available(
    ...
    mock_base_broker_instance.with_result_backend.return_value = mock_broker_with_backend
    ...
    mock_base_broker_instance.with_result_backend.assert_called_once_with(mock_backend_instance)
...
@mock.patch('taskiq_redis.RedisStreamBroker', side_effect=Exception("Redis connection failed"))
@mock.patch('taskiq_redis.RedisAsyncResultBackend')
def test_broker_fallbacks_to_inmemory_if_redis_fails_on_prod(
    ...
    with caplog.at_level(logging.ERROR, logger="core_sdk.broker.setup"): # <--- ИЗМЕНЕНИЕ УРОВНЯ
        broker = reload_broker_setup_module()
    ...
    assert any(
        "FAILED to initialize Redis Broker" in record.message and
...
def test_broker_fallbacks_if_redis_url_not_set_on_prod(
    ...
    mock_redis_broker_cls.side_effect = Exception("Cannot connect to default localhost Redis")

    with caplog.at_level(logging.INFO, logger="core_sdk.broker.setup"): # Ловим INFO и выше
        broker = reload_broker_setup_module()
