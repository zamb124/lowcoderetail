...
def test_create_access_token_no_secret_key_raises_error(
    sample_token_data: Dict[str, Any],
):
    with pytest.raises(ValueError, match="Secret key must be provided"):
        create_access_token(
            data=sample_token_data, secret_key="", expires_delta=SHORT_EXPIRES_DELTA
        )
...
def test_create_refresh_token_success(sample_token_data: Dict[str, Any]):
    ...
    token = create_refresh_token(
        data=refresh_data, secret_key=TEST_SECRET_KEY, expires_delta=LONG_EXPIRES_DELTA
    )
    assert isinstance(token, str)
...
def test_create_refresh_token_no_secret_key_raises_error(
    sample_token_data: Dict[str, Any],
):
    with pytest.raises(ValueError, match="Secret key must be provided"):
        create_refresh_token(
            data=sample_token_data, secret_key="", expires_delta=LONG_EXPIRES_DELTA
        )
...
def test_verify_token_expired_raises_error(sample_token_data: Dict[str, Any]):
    ...
    token = create_access_token(
        data=sample_token_data, secret_key=TEST_SECRET_KEY, expires_delta=expired_delta
    )
    with pytest.raises(
        ValueError, match="Could not validate credentials"
    ):  # Ожидаем credentials_exception по умолчанию
...
def test_verify_token_no_secret_key_raises_value_error(
    sample_token_data: Dict[str, Any],
):
    token = create_access_token(
        data=sample_token_data,
...
def test_verify_token_custom_credentials_exception(sample_token_data: Dict[str, Any]):
    ...
    with pytest.raises(CustomAuthError):
        verify_token(
            token,
            TEST_SECRET_KEY,
            credentials_exception=CustomAuthError("Custom message"),
        )
