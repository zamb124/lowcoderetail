...
from pydantic import BaseModel as PydanticBaseModel, HttpUrl, Field as PydanticField, ValidationError, ConfigDict
...
class RegTestCreateSchema(RegTestPydanticSchema): pass
class RegTestUpdateSchema(PydanticBaseModel):
    name: Optional[str] = None
    model_config = ConfigDict(extra='allow')
class RegTestReadSchema(RegTestModel): pass
...
class RegTestManager(BaseDataAccessManager): pass # type: ignore
class AnotherManager(BaseDataAccessManager): pass # type: ignore
...
def test_register_local_success_with_all_params():
    ModelRegistry.register_local(
        model_name="MyItem", model_cls=RegTestModel, manager_cls=RegTestManager,
        create_schema_cls=RegTestCreateSchema, update_schema_cls=RegTestUpdateSchema,
        read_schema_cls=RegTestReadSchema, filter_cls=RegTestFilter
    )
...
def test_register_remote_success():
    remote_conf = RemoteConfig(service_url=HttpUrl("http://test.com"), model_endpoint="/api/items")
    ModelRegistry.register_remote(
        model_name="MyRemoteItem", model_cls=RegTestPydanticSchema,
        config=remote_conf, create_schema_cls=RegTestCreateSchema,
        update_schema_cls=RegTestUpdateSchema, read_schema_cls=RegTestPydanticSchema
    )
...
def test_register_overwrites_existing_with_warning(caplog):
    ModelRegistry.register_local(model_name="Item", model_cls=RegTestModel, manager_cls=RegTestManager)
    ModelRegistry.register_local(model_name="Item", model_cls=RegTestModel, manager_cls=AnotherManager)
...
class RebuildableSQLModel(SQLModel, RebuildablePydantic): # Наследуем от Pydantic с моком
    model_rebuild: ClassVar[mock.Mock] = mock.Mock(name="SQLModelRebuildMock") # <--- СВОЙ МОК
    id: Optional[int] = None

class RebuildableFilter(BaseSQLAlchemyFilter, RebuildablePydantic): # Фильтр тоже Pydantic
    model_rebuild: ClassVar[mock.Mock] = mock.Mock(name="FilterRebuildMock") # <--- СВОЙ МОК
    class Constants: model = RebuildableSQLModel
...
def test_rebuild_models_calls_model_rebuild_on_schemas():
    ...
    class RebuildCreateSchema(PydanticBaseModel): model_rebuild: ClassVar[mock.Mock] = mock.Mock(name="CreateRebuildMock")
    class RebuildUpdateSchema(PydanticBaseModel): model_rebuild: ClassVar[mock.Mock] = mock.Mock(name="UpdateRebuildMock")
    class RebuildReadSchema(PydanticBaseModel): model_rebuild: ClassVar[mock.Mock] = mock.Mock(name="ReadRebuildMock")
...
def test_rebuild_models_handles_none_schemas():
    ...
    class UnusedPydantic(PydanticBaseModel): model_rebuild: ClassVar[mock.Mock] = mock.Mock(name="UnusedPydanticMock")
    UnusedPydantic.model_rebuild.reset_mock()


    ModelRegistry.register_local(model_name="RebuildNone", model_cls=RebuildableSQLModel)
...
def test_clear_registry():
    ModelRegistry.register_local(model_cls=RegTestModel)
    assert ModelRegistry.is_configured() is True; assert len(ModelRegistry._registry) > 0
    ModelRegistry.clear()
    assert ModelRegistry.is_configured() is False; assert not ModelRegistry._registry
