...
async def test_create_instance_defaults_and_values(db_session: AsyncSession):
    ...
    item = ConcreteTestModelOne(id=new_id, name="Test Item", lsn=test_lsn, company_id=company_uuid)
...
async def test_update_instance_modifies_updated_at(db_session: AsyncSession):
    item_id = uuid.uuid4()
    item_lsn = 10
    item = ConcreteTestModelOne(id=item_id, name="Initial", company_id=uuid.uuid4(), lsn=item_lsn)
    db_session.add(item)
    await db_session.commit()
    await db_session.refresh(item)
...
async def test_vars_field_stores_json_data(db_session: AsyncSession):
    ...
    item_id = uuid.uuid4()
    item_lsn = 20
    item = ConcreteTestModelOne(id=item_id, name="Vars Test", company_id=uuid.uuid4(), vars=custom_data, lsn=item_lsn)
...
async def test_company_id_storage(db_session: AsyncSession):
    ...
    item_id = uuid.uuid4()
    item_lsn = 40
    item = ConcreteTestModelOne(id=item_id, name="Company ID Test", company_id=company_id_val, lsn=item_lsn)
...
async def test_querying_by_base_model_fields(db_session: AsyncSession):
    ...
    item1 = ConcreteTestModelOne(id=item1_id, name="Query Item 1", company_id=company1, vars={"tag": "A"}, lsn=item1_lsn)
    item2 = ConcreteTestModelOne(id=item2_id, name="Query Item 2", company_id=company2, vars={"tag": "B"}, lsn=item2_lsn)
    item3 = ConcreteTestModelOne(id=item3_id, name="Query Item 3", company_id=company1, vars={"tag": "C"}, lsn=item3_lsn)
