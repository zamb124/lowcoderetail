...
from core_sdk.tests.conftest import Item as TestItemModel, ItemCreate, ItemUpdate, ItemRead
...
@pytest.fixture
def service_client(http_client: httpx.AsyncClient) -> RemoteServiceClient[TestItemModel, ItemCreate, ItemUpdate]:
...
@pytest.fixture
def service_client_with_auth(http_client: httpx.AsyncClient) -> RemoteServiceClient[TestItemModel, ItemCreate, ItemUpdate]:
    return RemoteServiceClient(
        base_url=HttpUrl(SERVICE_URL_STR),
        model_endpoint=MODEL_ENDPOINT_STR.lstrip('/'),
        model_cls=TestItemModel,
...
async def test_get_success(service_client: RemoteServiceClient, respx_mock: MockRouter):
    ...
    expected_data = {"id": str(item_id), "name": "Test Get", "lsn": 1, "description": None, "value": None}
...
async def test_get_not_found_returns_none(service_client: RemoteServiceClient, respx_mock: MockRouter):
    item_id = uuid.uuid4()
    respx_mock.get(f"{FULL_API_URL}/{item_id}").respond(404)
    result = await service_client.get(item_id)
    assert result is None

async def test_get_server_error_raises_service_communication_error(service_client: RemoteServiceClient, respx_mock: MockRouter):
    item_id = uuid.uuid4()
...
async def test_list_success(service_client: RemoteServiceClient, respx_mock: MockRouter):
    item1_id, item2_id = uuid.uuid4(), uuid.uuid4()
    mock_items_data = [
        {"id": str(item1_id), "name": "Item 1", "lsn": 1, "description": None, "value": None},
        {"id": str(item2_id), "name": "Item 2", "lsn": 2, "description": None, "value": None}
    ]
    ...
    respx_mock.get(FULL_API_URL).respond(200, json={"items": mock_items_data}) # API возвращает словарь
...
async def test_list_api_returns_direct_list(service_client: RemoteServiceClient, respx_mock: MockRouter):
    mock_items_data = [{"id": str(uuid.uuid4()), "name": "Direct List Item", "lsn": 3}]
    respx_mock.get(FULL_API_URL).respond(200, json=mock_items_data) # API возвращает просто список
...
async def test_list_params_are_sent(service_client: RemoteServiceClient, respx_mock: MockRouter):
    route = respx_mock.get(url__regex=rf"{FULL_API_URL}\?.*").respond(200, json=[])

    await service_client.list(limit=10, cursor=50, filters={"name": "test", "active": True, "ids": [uuid.uuid4(), uuid.uuid4()]})
    ...
    assert "ids=" in request_url # Проверяем, что параметр ids передается (httpx сделает ids=uuid1&ids=uuid2)

async def test_create_success(service_client: RemoteServiceClient, respx_mock: MockRouter):
    create_data = ItemCreate(name="New Item", description="Desc") # lsn и value будут None
    item_id = uuid.uuid4()
    response_data = {"id": str(item_id), "name": "New Item", "description": "Desc", "lsn": 1, "value": None}
    ...
    assert sent_payload.get("value") is None # Проверяем, что value None, если не передано

    assert isinstance(result, TestItemModel)
    assert result.id == item_id

async def test_update_success(service_client: RemoteServiceClient, respx_mock: MockRouter):
    item_id = uuid.uuid4()
    update_data = ItemUpdate(description="Updated")
    response_data = {"id": str(item_id), "name": "Original", "description": "Updated", "lsn": 2, "value": None}
...
async def test_delete_success_204(service_client: RemoteServiceClient, respx_mock: MockRouter):
    item_id = uuid.uuid4()
    respx_mock.delete(f"{FULL_API_URL}/{item_id}").respond(204)
    result = await service_client.delete(item_id)
    assert result is True

async def test_delete_success_404(service_client: RemoteServiceClient, respx_mock: MockRouter):
    item_id = uuid.uuid4()
    respx_mock.delete(f"{FULL_API_URL}/{item_id}").respond(404)
    result = await service_client.delete(item_id)
    assert result is True

async def test_delete_server_error_raises_exception(service_client: RemoteServiceClient, respx_mock: MockRouter):
    item_id = uuid.uuid4()
...
async def test_auth_header_is_sent(service_client_with_auth: RemoteServiceClient, respx_mock: MockRouter):
    item_id = uuid.uuid4()
    # Добавляем все поля TestItemModel в мок ответа
    route = respx_mock.get(f"{FULL_API_URL}/{item_id}").respond(200, json={"id": str(item_id), "name": "Auth Item", "lsn":1, "description":None, "value":None})
...
async def test_client_closes_if_owned():
    ...
    with mock.patch.object(client_owner._http_client, 'aclose', new_callable=mock.AsyncMock) as mock_aclose:
...
async def test_client_does_not_close_if_not_owned(http_client: httpx.AsyncClient):
    ...
    with mock.patch.object(http_client, 'aclose', new_callable=mock.AsyncMock) as mock_aclose_external:
