...
from core_sdk.tests.conftest import AppSetupTestSettings, worker_settings # Используем worker_settings из conftest
...
async def test_initialize_registry_config_called_and_configures(
    ...
    def side_effect_import_module(module_name):
        if module_name == test_module_name:
            ...
            ModelRegistry.register_local(model_cls=Item, model_name="DummyForWorkerSetupTest")
    ...
    await initialize_worker_context(settings=worker_settings, registry_config_module=test_module_name, rebuild_models=False)
...
async def test_shutdown_calls_close_db(
        mock_io_for_worker_setup: Dict[str, mock.Mock]
):
