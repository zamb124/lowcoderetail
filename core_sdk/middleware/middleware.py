...
class DBSessionMiddleware(BaseHTTPMiddleware):
    async def dispatch(
        ...
        async with managed_session():
            logger.debug(
                f"DBSessionMiddleware: Entered managed_session for {request.method} {request.url.path}"
            )
            try:
                ...
                logger.exception(
                    f"DBSessionMiddleware: Exception during request processing within managed_session for {request.method} {request.url.path}"
                )
                raise e  # Перевыбрасываем ошибку для стандартной обработки FastAPI
            finally:
                logger.debug(
                    f"DBSessionMiddleware: Exiting managed_session for {request.method} {request.url.path}"
                )
        return response
