...
from core_sdk.exceptions import ServiceCommunicationError # ConfigurationError не используется здесь
...
CreateSchemaType = TypeVar("CreateSchemaType", bound=PydanticBaseModel) # Схема для создания объекта
UpdateSchemaType = TypeVar("UpdateSchemaType", bound=PydanticBaseModel) # Схема для обновления объекта
...
class RemoteServiceClient(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    def __init__(
        ...
        logger.debug(f"RemoteServiceClient initialized for {self.api_base_url}. Owns client: {self._owns_client}")
    ...
    async def get(self, item_id: UUID) -> Optional[ModelType]:
        ...
        try:
            ...
            if e.status_code != 404: # Эта проверка может быть избыточной, если _request корректно обрабатывает allowed_statuses
                 logger.error(f"Failed to get item {item_id} from {url}: {e}", exc_info=True)
                 raise
    ...
    async def delete(self, item_id: UUID) -> bool:
        ...
        try:
            ...
            logger.info(f"Item {item_id} deleted (or was already not found) successfully from {url}.")
            return True
        except ServiceCommunicationError as e:
    ...
    async def close(self) -> None:
        ...
        if self._owns_client and self._http_client:
            ...
            try:
                ...
                logger.info(f"Owned HTTP client for {self.api_base_url} closed successfully.")
            except Exception as e:
                logger.error(f"Error closing owned HTTP client for {self.api_base_url}: {e}", exc_info=True)
