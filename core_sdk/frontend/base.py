...
async def get_modal_wrapper(
    ...
    content_url: str = Query(
        ..., description="URL для загрузки содержимого модального окна"
    ),
    modal_title: str = Query("Модальное окно", description="Заголовок модального окна"),
    modal_id: Optional[str] = Query(
        None, description="Опциональный ID для модального окна"
    ),
...
@router.post(
    "/resolve-titles", response_model=ResolveTitlesResponse, name="resolve_titles"
)
async def resolve_titles_endpoint(
    ...
    try:
        list_result = await manager.list(
            filters={"id__in": ids_to_resolve}, limit=len(ids_to_resolve) + 10
        )
    ...
    for item_id_val in ids_to_resolve:
        ...
        if not item:
            item = await manager.get(item_id_val)
        if item:
            item_title = next(
                (
                    str(getattr(item, fld))
                    for fld in title_field_candidates
                    if hasattr(item, fld) and getattr(item, fld)
                ),
                None,
            )
            resolved_titles[item_id_val] = (
                item_title or f"{model_name} {str(item_id_val)[:8]}..."
            )
        else:
            resolved_titles[item_id_val] = f"ID: {str(item_id_val)[:8]} (не найден)"
    return ResolveTitlesResponse(root=resolved_titles)


# --- РУЧКИ ДЛЯ ФОРМ И ПРЕДСТАВЛЕНИЙ (ВОЗВРАЩАЮТ ТОЛЬКО КОНТЕНТ) ---
@router.get(
    "/view/{model_name}/{item_id}", response_class=HTMLResponse, name="get_view"
)
...
@router.get(
    "/form/edit/{model_name}/{item_id}",
    response_class=HTMLResponse,
    name="get_edit_form",
)
async def get_edit_form_content(
    renderer: ViewRenderer = Depends(get_edit_form_renderer),
):
    return await renderer.render_to_response()
...
async def get_create_form_content(
    renderer: ViewRenderer = Depends(get_create_form_renderer),
):
    return await renderer.render_to_response()


@router.get("/list/{model_name}", response_class=HTMLResponse, name="get_list_view")
async def get_list_view_content(
    renderer: ViewRenderer = Depends(get_list_mode_renderer),
):
    return await renderer.render_to_response()
...
async def create_item(
    ...
    form_renderer: ViewRenderer = Depends(
        get_create_form_renderer
    ),  # form_renderer в режиме CREATE
):
    ...
    try:
        ...
        try:
            form_renderer.item = target_schema_cls()  # Пустой экземпляр для формы
        except Exception:
            form_renderer.item = None
        return await form_renderer.render_to_response(
            status_code=422
        )  # Возвращаем form.html с ошибкой

    try:
        new_item = await form_renderer.manager.create(json_data)
        response = Response(status_code=204, content=None)
        response.headers["HX-Trigger"] = (
            f"closeModal, itemCreated_{model_name}, refreshData"
        )
        return response
...
@router.put(
    "/item/{model_name}/{item_id}", response_class=HTMLResponse, name="update_item"
)
async def update_item(
    ...
    try:
        ...
        if form_renderer.item is None:
            await form_renderer._load_data()  # На всякий случай
        form_renderer.errors = {"_form": ["Неверный формат JSON."]}
        return await form_renderer.render_to_response(
            status_code=422
        )  # form.html с ошибкой
    ...
    try:
        ...
        if form_renderer.item and hasattr(form_renderer.item, "model_fields"):
            for (
                field_name
            ) in target_schema_cls.model_fields.keys():  # Итерируемся по полям схемы
                if hasattr(form_renderer.item, field_name) and hasattr(
                    instance_with_user_data, field_name
                ):
                    setattr(
                        instance_with_user_data,
                        field_name,
                        getattr(form_renderer.item, field_name),
                    )
...
@router.delete(
    "/item/{model_name}/{item_id}", response_class=Response, name="delete_item"
)
async def delete_item(renderer: ViewRenderer = Depends(get_view_mode_renderer)):
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=500, detail="Delete operation failed unexpectedly."
            )
    except HTTPException as e:
        raise e
    except Exception as e:
    ...
    if id:
        try:
            ...
            if item:
                ...
                return JSONResponse(
                    content=[
                        {"value": str(item.id), "label": label, "id": str(item.id)}
                    ]
                )
    ...
    try:
        ...
        for item in items:
            ...
            label = next(
                ...
                str(item_id_val) if item_id_val else "N/A",
            )
            if item_id_val:
                options_list.append(
                    {"value": str(item_id_val), "label": label, "id": str(item_id_val)}
                )
        from fastapi.responses import JSONResponse
...
@router.get(
    "/edit-field/{model_name}/{item_id}/{field_name}",
    response_class=HTMLResponse,
    name="get_inline_edit_field",
)
async def get_inline_edit_field(
    renderer: ViewRenderer = Depends(get_inline_edit_field_renderer),
):
...
async def update_inline_field(
    ...
    try:
        ...
        if field_name not in json_data:
            return HTMLResponse(
                f'<div class="text-danger p-1">Error: field {field_name} missing.</div>',
                status_code=400,
            )
    ...
    if error_renderer.item is None:
        await error_renderer._load_data()
    ...
    try:
        ...
        if field_name not in update_schema.model_fields:
            ...
            if error_renderer.item and hasattr(error_renderer.item, field_name):
                setattr(error_renderer.item, field_name, raw_value_from_json)
            return await error_renderer.render_field_to_response(
                field_name, status_code=400
            )

        field_info_obj = update_schema.model_fields[
            field_name
        ]  # Используем field_info_obj
        annotation = field_info_obj.annotation
        try:
            ValidatorModel = create_model(
                "FieldValidator", **{field_name: (annotation, ...)}
            )
            validated_field_data = ValidatorModel.model_validate(
                {field_name: raw_value_from_json}
            )
            validated_value = getattr(validated_field_data, field_name)
        except ValidationError as ve:
            error_messages = [
                e_detail.get("msg", "Invalid value.") for e_detail in ve.errors()
            ]
            error_renderer.errors = {field_name: error_messages}
            if error_renderer.item and hasattr(error_renderer.item, field_name):
                setattr(error_renderer.item, field_name, raw_value_from_json)
            return await error_renderer.render_field_to_response(
                field_name, status_code=422
            )

        updated_item = await manager.update(item_id, {field_name: validated_value})

        success_cell_renderer = ViewRenderer(
            request,
            model_name,
            dam_factory,
            user,
        ...
        if error_renderer.item and hasattr(error_renderer.item, field_name):
            setattr(error_renderer.item, field_name, raw_value_from_json)
        return await error_renderer.render_field_to_response(
            field_name, status_code=500
        )


@router.get(
    "/view-field/{model_name}/{item_id}/{field_name}",
    response_class=HTMLResponse,
    name="get_table_cell_view",
)
async def get_table_cell_view(
    renderer: ViewRenderer = Depends(get_table_cell_renderer),
):
    return await renderer.render_field_to_response(renderer.field_to_focus)


@router.get(
    "/list-rows/{model_name}", response_class=HTMLResponse, name="get_list_rows"
)
async def get_list_rows_content(
    renderer: ViewRenderer = Depends(get_list_rows_renderer),
):
...
async def get_filter_form_content(
