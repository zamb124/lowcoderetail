...
from typing import Any, Type, Optional as TypingOptional, List as TypingList, Union, get_origin, get_args, Dict as TypingDict
...
def get_list_item_type(annotation: Any) -> Any:
  ...
  if get_origin(possible_list_type) is TypingList or get_origin(possible_list_type) is list:
...
def is_pydantic_sqlmodel_type(type_to_check: Any) -> bool:
  if inspect.isclass(type_to_check):
    return issubclass(type_to_check, PydanticBaseModel) or issubclass(type_to_check, SQLModel)
  return False
...
def is_relation(annotation: Any) -> bool:
  ...
  if get_origin(base_type_of_annotation) is TypingList or get_origin(base_type_of_annotation) is list:
    # Если это список, получаем тип его элементов
    item_type = get_list_item_type(base_type_of_annotation) # get_list_item_type уже вернет базовый тип элемента
  ...
  if is_pydantic_sqlmodel_type(type_to_get_name_from):
    ...
    try:
      for name, info in ModelRegistry._registry.items():
        if info.model_cls == type_to_get_name_from or \
                info.read_schema_cls == type_to_get_name_from:
          return name
...
if __name__ == '__main__':
  class MyModel(SQLModel): id: int
  class AnotherModel(PydanticBaseModel): name: str
