...
SDK_TEMPLATES_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "templates")
...
def initialize_templates(service_template_dir: str):
    ...
    if not os.path.isdir(service_template_dir):
         logger.warning(f"Service template directory '{service_template_dir}' not found. Only SDK templates will be available.")
...
def get_templates() -> Jinja2Templates:
    if templates is None:
        ...
        raise RuntimeError("Templates not initialized. Call initialize_templates() during app startup.")
    return templates