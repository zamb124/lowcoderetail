    ...
    async with AsyncExitStack() as stack:
        if manage_http_client:
            try:
                ...
                logger.info(
                    "SDK Lifespan: Entered global_http_client_lifespan context."
                )
            except Exception as e:
                logger.critical(
                    "SDK Lifespan: Failed to enter global_http_client_lifespan context.",
                    exc_info=True,
                )
        ...
        try:
            ...
            logger.info(
                "SDK Lifespan: Database initialized and close_db registered for shutdown."
            )
        except Exception as e:
            logger.critical(
                "SDK Lifespan: Database initialization failed.", exc_info=True
            )
        ...
        if enable_broker:
            ...
            if broker and hasattr(broker, "startup") and callable(broker.startup):
                ...
                logger.warning(
                    f"SDK Lifespan: Broker '{type(broker).__name__}' does not have a startup method or is not callable. Skipping startup."
                )
            else:
        ...
        if rebuild_models:
            ...
            try:
                ...
                if schemas_to_rebuild:
                    logger.debug(
                        f"Explicitly rebuilding {len(schemas_to_rebuild)} provided schemas..."
                    )
                    rebuilt_count = 0
                    for schema_cls in schemas_to_rebuild:
                        if (
                            ...
                            and hasattr(schema_cls, "model_rebuild")
                        ):
                            try:
                                schema_cls.model_rebuild(force=True)
                                logger.debug(
                                    f"Rebuilt: {schema_cls.__module__}.{schema_cls.__name__}"
                                )
                                rebuilt_count += 1
                            except Exception as rebuild_err:
                                logger.error(
                                    f"Error explicitly rebuilding schema {schema_cls.__name__}: {rebuild_err}",
                                    exc_info=True,
                                )
                        else:
                            logger.warning(
                                f"Item in schemas_to_rebuild is not a valid Pydantic model with rebuild method: {schema_cls}"
                            )
                    logger.debug(
...
def create_app_with_sdk_setup(
    ...
    if enable_auth_middleware:
        ...
        app.add_middleware(
            ...
            ),  # ALGORITHM должен быть в settings
            allowed_paths=list(combined_allowed_paths),
        )
        logger.info(
            f"AuthMiddleware added. Allowed paths: {sorted(list(combined_allowed_paths))}"
        )
    else:
    ...
    if cors_origins_config:
        if isinstance(cors_origins_config, str):
            ...
            origins = [
                str(origin).strip()
                for origin in cors_origins_config
                if str(origin).strip()
            ]
        ...
        logger.info(
            f"CORS middleware enabled for origins: {'*' if allow_all else origins}"
        )
    else:
        logger.warning(
            "CORS middleware is disabled (BACKEND_CORS_ORIGINS not set in settings)."
        )
    if enable_auth_middleware:
        ...
        # ----------------------------------------------------
        logger.info(
            f"AuthMiddleware added. Allowed paths: {sorted(list(default_allowed_set))}"
        )
    else:
        logger.info("AuthMiddleware is disabled by configuration.")
    ...
    for router_instance in api_routers:
        if isinstance(router_instance, APIRouter):
            ...
            logger.warning(
                f"Item in api_routers is not an APIRouter instance: {type(router_instance)}. Skipping."
            )
    app.include_router(main_api_router)
    logger.info(
        f"All provided API routers included under prefix: {settings.API_V1_STR}"
    )
