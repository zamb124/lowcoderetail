...
async def initialize_worker_context(
    ...
    try:
        ...
        worker_pool_size = int(os.getenv("WORKER_DB_POOL_SIZE", str(getattr(settings, 'DB_POOL_SIZE', 5))))
        worker_max_overflow = int(os.getenv("WORKER_DB_MAX_OVERFLOW", str(getattr(settings, 'DB_MAX_OVERFLOW', 2))))
        ...
        logger.info(f"SDK Database initialized for worker (pool_size={worker_pool_size}).")
    except Exception as e:
        logger.critical("Failed to initialize SDK Database for worker.", exc_info=True)
        raise RuntimeError("Database initialization failed, worker cannot start.") from e

    # 2. Конфигурация ModelRegistry (через импорт модуля)
    if registry_config_module:
        logger.info(f"Configuring ModelRegistry by importing module: {registry_config_module}")
        try:
            importlib.import_module(registry_config_module)
            if not ModelRegistry.is_configured():
                 logger.warning(f"Module {registry_config_module} imported, but ModelRegistry is still not configured.")
            else:
                 logger.info("ModelRegistry configured successfully for worker.")
        except ImportError:
            logger.error(f"Could not import registry configuration module: {registry_config_module}")
            # Решите, критично ли это для воркера
            # raise RuntimeError(f"Failed to import registry config module: {registry_config_module}")
        except Exception as e:
            logger.error(f"Error configuring ModelRegistry from module {registry_config_module}.", exc_info=True)
            # raise RuntimeError(f"Failed to configure ModelRegistry: {e}") from e
    else:
        logger.warning("No registry_config_module provided. ModelRegistry might not be configured for worker tasks.")
    ...
    if rebuild_models:
        ...
        try:
            ...
            logger.error("Error during Pydantic/SQLModel model rebuild for worker.", exc_info=True)
    else:
