...
def init_db(
    ...
    logger.info(f"Initializing database engine and session maker for URL: {database_url[:database_url.find('@') + 1]}********")

    options_to_pass = engine_options.copy() if engine_options else {} # Копируем, чтобы не изменять оригинал
    ...
    if pool_class_in_options and pool_class_in_options in [StaticPool, NullPool]:
        ...
        logger.debug(f"Using specified poolclass {pool_class_in_options.__name__}. "
                     f"Removed pool_size/max_overflow from engine options if they were present.")

    try:
        _db_engine = create_async_engine(database_url, echo=echo, future=True, **options_to_pass) # Передаем отфильтрованные options
