...
from core_sdk.config import BaseAppSettings, SettingsConfigDict # Используем базовые настройки SDK
...
logger.info("Effective .env file path for Frontend: %s", _EFFECTIVE_ENV_FILE_PATH_VAR_LOCAL)
if not os.path.exists(_EFFECTIVE_ENV_FILE_PATH_VAR_LOCAL):
    logger.warning(".env file for Frontend not found at %s. Using environment variables only.", _EFFECTIVE_ENV_FILE_PATH_VAR_LOCAL)
...
class FrontendSettings(BaseAppSettings):
    ...
    REDIS_URL: Optional[RedisDsn] = Field(None, description="URL для Redis (для WebSocket/Taskiq).")
    ALGORITHM: str = Field("HS256", description="Алгоритм подписи JWT токенов.") # Нужен для middleware

    # --- URLs других сервисов ---
    # Важно: Это базовые URL сервисов, без /api/v1
    CORE_SERVICE_URL: HttpUrl = Field(..., description="Базовый URL к Core сервису (e.g., http://core:8000).")
...
try:
    ...
    logger.critical("Failed to load settings for Frontend from '%s'.", _EFFECTIVE_ENV_FILE_PATH_VAR_LOCAL, exc_info=True)
