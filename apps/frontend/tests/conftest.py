...
from frontend.app.config import FrontendSettings as AppSettingsClass # Настройки Frontend
...
from core_sdk.db.session import init_db as sdk_init_db, close_db as sdk_close_db # Для жизненного цикла SDK
...
def test_settings(set_test_environment_var) -> AppSettingsClass:
    ...
    try:
        ...
        print(f"Test settings loaded: PROJECT_NAME={settings.PROJECT_NAME}, CORE_URL={settings.CORE_SERVICE_URL}")
        return settings
...
async def manage_sdk_lifecycles(test_settings: AppSettingsClass):
    ...
    async with get_global_http_client(manage=True): # Используем менеджер контекста из SDK
        ...
        try:
            ...
            assert ModelRegistry.is_configured(), "ModelRegistry failed to configure in tests!"
...
async def async_client_frontend(test_settings: AppSettingsClass, manage_sdk_lifecycles) -> AsyncGenerator[AsyncClient, None]:
    ...
    async with AsyncClient(transport=transport, base_url="http://testfrontend") as client:
        yield client
...
@pytest.fixture(scope="function")
def auth_headers(test_settings: AppSettingsClass, test_user_auth_data: Dict[str, Any]) -> Dict[str, str]:
...
@pytest.fixture(scope="function")
def superuser_auth_headers(test_settings: AppSettingsClass, test_superuser_auth_data: Dict[str, Any]) -> Dict[str, str]:
