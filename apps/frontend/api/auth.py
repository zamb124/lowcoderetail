...
from data_access import get_global_http_client # Используем get_global_http_client из SDK
...
async def proxy_login_for_access_token(
    ...
    core_api_prefix = getattr(settings, 'CORE_API_V1_STR', '/api/v1') # Фоллбэк на /api/v1
    ...
    try:
        ...
        if refresh_token:
            refresh_max_age = settings.REFRESH_TOKEN_EXPIRE_MINUTES * 60 # Предполагаем, что есть такая настройка
            response.set_cookie(
                key="refresh_token", value=refresh_token, max_age=refresh_max_age, expires=refresh_max_age,
                path=f"{router.prefix}/refresh", httponly=True, samesite="lax", secure=settings.ENV != "dev", # Используем префикс роутера
            )
...
@router.post("/logout", status_code=status.HTTP_200_OK, include_in_schema=False, name="handle_logout")
async def handle_logout(response: Response):
    logger.info("Processing logout request.")
    response.delete_cookie(key="Authorization", path="/", httponly=True, samesite="lax")
    response.delete_cookie(key="refresh_token", path=f"{router.prefix}/refresh", httponly=True, samesite="lax")
