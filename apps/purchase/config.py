...
class Settings(BaseAppSettings):
    ...
    SECRET_KEY: str = Field(..., description="Секретный ключ для сервиса Purchase (например, для подписи чего-либо специфичного).")

    # URL других сервисов, от которых может зависеть Purchase
    CORE_SERVICE_URL: Optional[str] = Field("http://core:8000", description="URL к Core сервису.") # Убедимся, что есть значение
...
try:
    ...
    logger.critical("Failed to load settings for PurchaseService from '%s'.", _EFFECTIVE_ENV_FILE_PATH_VAR_LOCAL, exc_info=True)
