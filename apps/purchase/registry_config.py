...
def configure_purchase_registry():
    ...
    try:
        if ModelRegistry.is_configured() and ModelRegistry.get_model_info(model_to_register):
            logger.warning(
                f"Model '{model_to_register}' already registered. Skipping local configuration for {service_name} service."
            )
        else:
    ...
    try:
        if ModelRegistry.is_configured() and ModelRegistry.get_model_info(remote_company_model_name):
            logger.warning(
                f"Remote model '{remote_company_model_name}' already registered. Skipping remote configuration."
            )
        else:
            if not purchase_settings.CORE_SERVICE_URL:
                logger.error(
                    f"CORE_SERVICE_URL is not set in Purchase settings. Cannot register remote model '{remote_company_model_name}'."
                )
            else:
                logger.info(
                    f"Registering remote model '{remote_company_model_name}' (from Core service at {purchase_settings.CORE_SERVICE_URL})..."
                )
