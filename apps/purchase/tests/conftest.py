...
def set_test_environment_var():
    ...
    if "ENV" in os.environ:
        del os.environ["ENV"]
    if "PORT_PURCHASE" in os.environ:
        del os.environ["PORT_PURCHASE"]
...
@pytest_asyncio.fixture(
    scope="session"
)  # Убрал autouse=True, чтобы не конфликтовать с core/conftest.py при совместном запуске
...
async def db_session_purchase(
    manage_service_db_for_tests_purchase,
) -> AsyncGenerator[None, None]:  # Зависит от переименованной фикстуры
...
@pytest_asyncio.fixture(scope="session")
async def core_superuser_token(
    test_settings: AppSettingsClass, external_http_client: httpx.AsyncClient
) -> Optional[str]:
    """
    ...
    print(
        f"Attempting to login to Core service at {login_url} as {core_admin_email} to get token for Purchase tests..."
    )
    try:
        response = await external_http_client.post(
            login_url, data={"username": core_admin_email, "password": core_admin_password}
        )
...
async def dam_factory_purchase_test(
    ...
    return DataAccessManagerFactory(
        registry=ModelRegistry, http_client=external_http_client, auth_token=core_superuser_token
    )


@pytest_asyncio.fixture(scope="function")
async def async_client_purchase(
...
@pytest_asyncio.fixture
async def test_purchase_order_item(
    dam_factory_purchase_test: DataAccessManagerFactory, db_session_purchase
) -> PurchaseOrder:
