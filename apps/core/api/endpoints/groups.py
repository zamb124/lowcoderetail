...
from fastapi import Depends, HTTPException, status, Body  # APIRouter и Body не нужны здесь напрямую
...
async def assign_permission_codename_to_group(
    group_id: UUID,
    permission_codename: str = Body(..., embed=True, description="Коднайм права для добавления."),
    ...
    if permission_codename not in group.permissions:
        ...
        try:
            ...
            updated_group = await group_manager.update(group_id, {"permissions": current_perms})
            logger.info(f"Permission '{permission_codename}' assigned to group {group_id}.")
            return updated_group
        except Exception as e:
            logger.exception(f"Failed to assign permission '{permission_codename}' to group {group_id}")
            raise HTTPException(status_code=500, detail="Could not assign permission.")
    else:
        logger.info(f"Permission '{permission_codename}' already assigned to group {group_id}.")
...
async def revoke_permission_codename_from_group(
    ...
    if permission_codename in group.permissions:
        ...
        try:
            updated_group = await group_manager.update(group_id, {"permissions": current_perms})
            logger.info(f"Permission '{permission_codename}' revoked from group {group_id}.")
            return updated_group
        except Exception as e:
            logger.exception(f"Failed to revoke permission '{permission_codename}' from group {group_id}")
            raise HTTPException(status_code=500, detail="Could not revoke permission.")
    else:
        logger.info(f"Permission '{permission_codename}' was not assigned to group {group_id}.")
        return group