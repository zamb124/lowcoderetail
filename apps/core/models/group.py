...
from sqlalchemy.dialects.postgresql import (
    UUID as PG_UUID,
)  # Оставляем PG_UUID для ForeignKey
...
class Group(BaseModelWithMeta, table=True):
    ...
    __table_args__ = (
        UniqueConstraint("company_id", "name", name="uq_group_company_id_name"),
    )
    ...
    company_id: uuid.UUID = Field(
        ...
        sa_column=Column(
            ...
            ),  # При удалении компании удаляются и ее группы
            nullable=False,
            index=True,
        ),
...
class GroupFilter(DefaultFilter):
    ...
    name: Optional[str] = Field(
        default=None, description="Фильтр по точному названию группы."
    )
    name__like: Optional[str] = Field(
        default=None, description="Фильтр по части названия группы (регистрозависимый)."
    )
