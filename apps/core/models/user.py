...
class User(BaseModelWithMeta, table=True):
    ...
    company_id: Optional[uuid.UUID] = (
        Field(  # Тип Optional, так как ondelete="SET NULL"
            sa_column=Column(
    ...
    first_name: Optional[str] = Field(
        default=None, max_length=100, description="Имя пользователя."
    )
    last_name: Optional[str] = Field(
        default=None, max_length=100, description="Фамилия пользователя."
    )
    ...
    company: Optional["Company"] = (
        Relationship(  # Optional, т.к. company_id может быть NULL
...
class UserFilter(DefaultFilter):
    ...
    email: Optional[str] = Field(
        default=None, description="Фильтр по точному email адресу."
    )
    email__in: Optional[List[str]] = Field(
        default=None, description="Фильтр по списку email адресов."
    )
    email__like: Optional[str] = Field(
        default=None, description="Фильтр по части email адреса (регистрозависимый)."
    )
    first_name__like: Optional[str] = Field(
        default=None, description="Фильтр по части имени."
    )
    last_name__like: Optional[str] = Field(
        default=None, description="Фильтр по части фамилии."
    )
    is_active: Optional[bool] = Field(
        default=None, description="Фильтр по статусу активности (true/false)."
    )
    is_superuser: Optional[bool] = Field(
        default=None, description="Фильтр по статусу суперпользователя (true/false)."
    )
