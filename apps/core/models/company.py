...
class Company(BaseModelWithMeta, table=True):
    ...
    address: Optional[str] = Field(
        default=None,
        description="Юридический или фактический адрес компании."
    )
    ...
    company_id: Optional[uuid.UUID] = Field(
        default=None,
        sa_column=Column("company_id", PG_UUID(as_uuid=True), index=True, nullable=True), # nullable=True может быть неверным для Company
...
class CompanyFilter(DefaultFilter):
    ...
    """
    name: Optional[str] = Field(default=None, description="Фильтр по точному названию компании.")
    name__like: Optional[str] = Field(default=None, description="Фильтр по части названия компании (регистрозависимый).")
    name__in: Optional[List[str]] = Field(default=None, description="Фильтр по списку точных названий компаний.")
    is_active: Optional[bool] = Field(default=None, description="Фильтр по статусу активности (true/false).")
    vat_id: Optional[str] = Field(default=None, description="Фильтр по точному ИНН/VAT ID.")
    vat_id__isnull: Optional[bool] = Field(default=None, description="Фильтр по наличию ИНН/VAT ID (true - IS NULL, false - IS NOT NULL).")
