...
class CompanyUpdate(SQLModel):
    ...
    address: Optional[str] = Field(
        default=None,
        description="Новый адрес компании."
    )
    vat_id: Optional[str] = Field(
        default=None,
        max_length=50,
        description="Новый ИНН/VAT ID компании."
    )
...
class CompanyReadWithDetails(CompanyRead):
    ...
    users: List[UserRead] = Field(
        default=[],
        description="Список пользователей, принадлежащих этой компании."
    )
    groups: List[GroupRead] = Field(
        default=[],
        description="Список групп, принадлежащих этой компании."
    )

logger.debug("Company schemas defined: CompanyBase, CompanyCreate, CompanyUpdate, CompanyRead, CompanyReadWithDetails")