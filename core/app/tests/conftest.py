...
from core.app.config import Settings as AppSettingsClass
from core.app.main import app as fastapi_app
# Модели и конфигурация Registry
from core.app import models
from core_sdk.registry import ModelRegistry
from core.app.registry_config import configure_core_registry
# Компоненты SDK для БД и DAM
from core_sdk.db.session import init_db, close_db, managed_session, get_session_dependency # Используем новую зависимость
from core_sdk.data_access import DataAccessManagerFactory, get_dam_factory, get_optional_token, get_global_http_client
...
def set_test_environment():
    ...
    modules_to_reload = [
        ...
        'core.app.config'        # Перезагружаем, чтобы он прочитал .env.test (если он зависит от ENV)
    ]
...
def test_settings(set_test_environment) -> AppSettingsClass: # Зависит от установки env var
    """Загружает настройки из .env.test или переменных окружения."""
    # Путь к .env.test
    env_file_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', '.env.test'))
    ...
    try:
        ...
        print(f"Test settings loaded successfully: PROJECT_NAME={loaded_settings.PROJECT_NAME}")
        # Проверка критичных настроек
        if not getattr(loaded_settings, 'DATABASE_URL', None): pytest.fail(f"TEST DATABASE_URL not loaded. Check {env_file_path}.")
        if not getattr(loaded_settings, 'REDIS_URL', None): pytest.fail(f"TEST REDIS_URL not loaded. Check {env_file_path}.")
        if not getattr(loaded_settings, 'SECRET_KEY', None): pytest.fail(f"TEST SECRET_KEY not loaded. Check {env_file_path}.")
    except Exception as e:
...
async def manage_sdk_db_lifecycle(test_engine: AsyncEngine, apply_migrations, test_settings): # Зависит от engine и миграций
...
@pytest_asyncio.fixture(scope="function")
async def test_company(dam_factory_test: DataAccessManagerFactory) -> models.company.Company:
    ...
    async with managed_session(): # Управляем сессией здесь
        try:
            company_manager: CompanyDataAccessManager = dam_factory_test.get_manager("Company")
...
async def superuser_token_headers(
    ...
    try:
        ...
        pytest.fail(f"Could not log in superuser '{superuser_email}'. Error: {e}, Response: {getattr(response, 'text', 'N/A')}")
...
async def normal_user_token_headers(
    ...
    try:
        ...
        return headers
         ...
         pytest.fail(f"Could not log in normal user '{normal_user_email}'. Error: {e}, Response: {getattr(response, 'text', 'N/A')}")

# --- Фикстура для тестового брокера Taskiq ---
@pytest.fixture(scope="session")
def test_taskiq_broker(set_test_environment) -> AsyncBroker: # Зависит от установки env var
    """
