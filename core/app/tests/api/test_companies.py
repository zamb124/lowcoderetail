...
from core.app import schemas, models # Импортируем схемы и модели
...
async def test_list_companies_filter_by_name(
    ...
    await async_client.post(COMPANIES_ENDPOINT, headers=superuser_token_headers, json={"name": other_name})
...
async def test_delete_company_success(
    ...
    response = await async_client.delete(
        f"{COMPANIES_ENDPOINT}/{company_id}",
        headers=superuser_token_headers
    )
...
async def test_activate_company_and_wait_timeout(
    ...
    async with managed_session():
        company_manager = dam_factory_test.get_manager("Company")
        company_data = schemas.company.CompanyCreate(name=f"Timeout Company {uuid4()}", is_active=False)
        db_company = await company_manager.create(company_data)
        company_id = db_company.id
...
async def test_list_companies_filter_is_active(
    ...
    async with managed_session():
        manager = dam_factory_test.get_manager("Company")
        c_active1 = await manager.create({"name": f"{base_name}_Active1", "is_active": True})
        c_active2 = await manager.create({"name": f"{base_name}_Active2", "is_active": True})
        c_inactive = await manager.create({"name": f"{base_name}_Inactive", "is_active": False})
    ...
    assert all(item["is_active"] for item in data_active["items"] if item["id"] in [str(c_active1.id), str(c_active2.id)])
    ...
    assert all(not item["is_active"] for item in data_inactive["items"] if item["id"] == str(c_inactive.id))
...
async def test_list_companies_filter_vat_id_isnull(
    ...
    async with managed_session():
        manager = dam_factory_test.get_manager("Company")
        c_with_vat = await manager.create({"name": f"{base_name}_With", "vat_id": "12345"})
        c_without_vat = await manager.create({"name": f"{base_name}_Without", "vat_id": None})
