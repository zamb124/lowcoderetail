...
async def test_get_user_by_id_forbidden(
    ...
    response = await async_client.get(
        f"{USERS_ENDPOINT}/{test_superuser.id}",
...
async def test_list_users_success(
    ...
    print("\nTesting LIST ASC")
    response_asc = await async_client.get(USERS_ENDPOINT, headers=superuser_token_headers, params={"limit": 1})
    ...
    assert next_cursor_asc == first_user_lsn # Для limit=1 курсор равен lsn единственного элемента
    ...
    assert all(item["lsn"] > next_cursor_asc for item in content_asc_next["items"]) # Проверяем LSN
    ...
    assert next_cursor_desc == last_user_lsn # Курсор для desc - это lsn первого (самого нового) элемента
    ...
    response_desc_prev = await async_client.get(
        USERS_ENDPOINT,
        headers=superuser_token_headers,
        params={"limit": 10, "direction": "desc", "cursor": next_cursor_desc} # Используем курсор
    )
...
async def test_update_user_change_password(
    ...
    async with managed_session():
        ...
        assert not verify_password(test_user._test_password, updated_user.hashed_password)
...
async def test_list_users_filter_is_active(
    ...
    async with managed_session():
        ...
        u_active = await manager.create({
            "email": email_active, "password": "password",
            "company_id": test_company.id, "is_active": True
        })
        u_inactive = await manager.create({
            "email": email_inactive, "password": "password",
            "company_id": test_company.id, "is_active": False
        })
...
async def test_list_users_filter_company_id(
    ...
    async with managed_session():
        ...
        user_in_comp2 = await user_manager.create({
            "email": f"user_comp2_{uuid4()}@example.com", "password": "password",
            "company_id": company2.id, "is_active": True
        })
